...
3. Lint tracked shell scripts in three passes: entry scripts with `--norc --exclude=SC1091 -S info -P SCRIPTDIR -P .`, sourced `modules/*` files with the same settings plus `SC2317` excluded, and `spec/*` test files with `SC2317` excluded.
